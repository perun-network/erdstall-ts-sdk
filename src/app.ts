...
export class AppInternals implements SigVerifier {
	...
	async verifySig(sig: SignedMessage): Promise<Uint8Array | undefined> {
		...
		if (sig.signature === undefined) return undefined;
...
export class L2Identity {
	...
	set aes(v: AESGCMKey | undefined) {
		this.#encryption = v;
	}

	set wildcardId(w: WildcardAddress) {
		this.#wildcardId = w;
	}

	constructor(
		wildcard: WildcardAddress | undefined,
		encryption: AESGCMKey | undefined,
		v?: {
	...
	get any_signer(): Signer | undefined {
		if (this.#eth instanceof EthereumSigner) return this.#eth;
		else if (this.#subst instanceof SubstrateSigner) return this.#subst;
		else return undefined;
	}

	get has_aes(): boolean {
		return this.#encryption !== undefined;
	}
	...
	set encryption(key: AESGCMKey) {
		this.#encryption = key;
	}
	set eth(v: EthereumSigner | EthereumAddress) {
		this.#eth = v;
	}
	set subst(v: SubstrateSigner | SubstrateAddress) {
		this.#subst = v;
	}

	get eth_addr(): EthereumAddress | undefined {
		if (this.#eth instanceof EthereumSigner) return this.#eth.address();
		else if (this.#eth instanceof EthereumAddress) return this.#eth;
		else return undefined;
	}

	get subst_addr(): SubstrateAddress | undefined {
		if (this.#subst instanceof SubstrateSigner) return this.#subst.address();
		else if (this.#subst instanceof SubstrateAddress) return this.#subst;
		else return undefined;
	}
...
export class App {
	...
	get initialized() {
		return this.#internals.initialized;
	}

	get chainTypes(): Map<Chain, string> {
		return new Map<Chain, string>(
			this.#internals.config!.chains.map((cfg) => [cfg.id, cfg.data.type()]),
		);
	}
	...
	constructor(enclaveConn: Enclave | URL, internals?: AppInternals) {
	...
	initialize(timeout?: number): Promise<this> {
		return new Promise((resolve, reject) => {
			const rejectTimeout = setTimeout(reject, timeout ? timeout! : 15_000);
