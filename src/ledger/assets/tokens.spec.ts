...
describe("Tokens", () => {
	...
	it("can sub tokens", () => {
		...
		let t2 = new Tokens([321n]);
		t1.sub(t2);
		expect(t1.value, "subbing compatible tokens should work").to.eql([123n]);
		expect(() => {
			t1.sub(t2);
	...
	it("compares tokens", () => {
		...
		expect(new Tokens([125n]).cmp(new Tokens([124n]))).to.eql("uncomparable");
		...
		expect(new Tokens([123n, 124n, 125n]).cmp(new Tokens([123n, 124n]))).to.eql(
			"gt",
		);
		expect(
