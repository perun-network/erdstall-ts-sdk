...
import { addressKey } from "#erdstall/ledger";

const testAssetsWithChecksum =
...
describe("Assets", () => {
	it("can be de- & encoded from & to JSON", () => {
		const jsonAssetsWithChecksum: { [key: string]: any } = JSON.parse(
			testAssetsWithChecksum,
		);
		const jsonAssetsNoChecksum = Object.fromEntries(
			Object.entries(jsonAssetsWithChecksum).map(([k, v]) => [
				addressKey(k),
				v,
			]),
		);
		const vs = Assets.fromJSON(jsonAssetsWithChecksum);
		expect(vs).eql(Assets.fromJSON(Assets.toJSON(vs)));
		expect(Assets.toJSON(vs)).eql(jsonAssetsNoChecksum);
	});
});
