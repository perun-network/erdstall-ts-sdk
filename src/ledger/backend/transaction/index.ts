...
export class ApproveTx extends WildcardTx {
	...
	override clone(): this {
		return new ApproveTx(this.chain, this.destination, this.amount) as this;
	}
...
export abstract class UnsignedTx {
	...
	get description(): WildcardTx {
		return this.#tx.clone();
	}
	abstract get native(): any;

	constructor(tx: WildcardTx) {
		this.#tx = tx.clone();
	}
...
class Mutex {
	...
	async session<T>(the_thing: (sessionId: any) => Promise<T>): Promise<T> {
		let id = await this.lock();

		try {
			return await the_thing(id);
		} finally {
			this.unlock(id);
		}
	}
...
export abstract class TxSigner {
	...
	constructor(chain: Chain, getPendingNonce: () => Promise<bigint>) {
		this.#chain = chain;
	...
	protected require(id: any, chain: Chain): void {
		...
		if (this.#chain !== chain)
			throw new Error(
				`Logic error: attempted to use TxSigner for ${getChainName(this.#chain)} on transaction for ${getChainName(chain)}.`,
			);
	}
	...
	async incNonce(sessId: any, chain: Chain): Promise<bigint> {
		...
		if (this.#nonce === undefined) this.#nonce = await this.#getPendingNonce();
		return this.#nonce++;
	}
	...
	async resetNonce(session: any, chain: Chain): Promise<void> {
		...
		if (!is_init) this.#resetHandlers.emit(this.#nonce);
	}
	...
	finishSigningSession(session: any): void {
		this.#signing.unlock(session);
	}
...
export abstract class TxSender {
	...
	require(chain: Chain): void {
		if (this.#chain !== chain)
			throw new Error(
				`Logic error: attempted to use TxSender for ${getChainName(this.#chain)} on transaction for ${getChainName(chain)}.`,
			);
	}
}
...
export class UnsignedTxBatch {
	...
	at(i: number): UnsignedTx {
		return this.#txs[i];
	}

	constructor(txs: UnsignedTx[]) {
		if (!txs.length) throw new Error("Transaction batches cannot be empty.");
		this.#chain = txs[0].chain;
		for (let tx of txs) {
			if (tx.chain !== this.#chain)
				throw new Error(
					"Tranaction batch contains transactions for multiple chains",
				);
		}
		this.#txs = [...txs];
	}
	...
	async sign(s: TxSigner, on_invalidate: () => void): Promise<SignedTxBatch> {
		return await s.signing_session(async (session: any) => {
			...
			for (let tx of this.#txs) signed_txs.push(await tx.sign(s, session));
...
export class SignedTxBatch {
	...
	get sender(): Address {
		return this.#txs[0].sender;
	}
	get startnonce(): bigint {
	...
	constructor(txs: SignedTx[], on_invalidate: () => void) {
		...
		for (let tx of txs) {
			...
			if (!tx.sender.equals(sender))
				throw new Error(
					"Signed transaction batch contains transactions from multiple senders",
				);
			++nonce;
		}
	...
	async send(signer: TxSigner, sender: TxSender): Promise<TxReceiptBatch> {
		...
		return await signer.signing_session(async (s: any) => {
			for (let tx of this.#txs) receipts.push(tx.send(sender));
...
export class TxReceiptBatch {
	...
	constructor(receipts: TxReceipt[]) {
		if (!receipts.length) throw new Error("Receipt batches cannot be empty.");
	...
	async awaitAll(): Promise<boolean> {
		return (await Promise.all(this.#receipts.map((r) => r.success))).every(
			(x) => x,
		);
	}
}
