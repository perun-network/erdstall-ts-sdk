...
export class UnsignedSubstrateTransaction extends UnsignedTx {
	...
	override async sign(s: TxSigner, session: any): Promise<SignedTx> {
		...
		return new SignedSubstrateTransaction(this.description, signed, s.address);
	}
}
...
export class SignedSubstrateTransaction extends SignedTx {
	...
	override get sender(): SubstrateAddress {
		return this.#sender;
	}
	...
	override get nonce(): bigint {
		return this.#native.nonce.toBigInt();
	}
	...
	override async send(s: TxSender): Promise<TxReceipt> {
		...
		const unlisten = await this.#native.send(
			(update: {
				txHash: any;
				events?: any[];
				status: TxStatus;
	...
	for (let arg in args) {
		...
		for (let field in args[arg])
			if (!(field in apiArgs[arg]))
				errors.push(
					`We passed field ${arg}.${field}, but it does not exist in the API.`,
				);
		for (let field in apiArgs[arg])
			if (!(field in args[arg]))
				errors.push(
					`Field ${arg}.${field} was not passed, so it got defaulted`,
				);
	}
...
export class API {
	...
	deposit(kind: AssetID, amount: Asset): UnsignedTx[] {
		...
		switch (kind.type()) {
			case AssetType.Fungible:
				{
					...
					txs.push(
						new UnsignedSubstrateTransaction(
			...
			case AssetType.NFT:
				...
				break;
			default:
				throw new Error(`unhandled asset type ${AssetTypeName(kind.type())}`);
		}
	...
	withdraw({
		...
		if (assets.length != 1)
			throw new Error(
				`Expected BP chunk to have only one asset, has ${assets.length}`,
			);
			...
			case AssetType.NFT:
				{
					...
					if (amount.value.length != 1)
						throw new Error(
							`Expected BP chunk to have one NFT, has ${amount.value.length}`,
						);
