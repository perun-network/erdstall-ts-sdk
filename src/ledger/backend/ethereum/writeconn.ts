...
export class LedgerConn {
	...
	private constructor(
		...
		this.#erdstall = Erdstall__factory.connect(contract.toString(), provider);
	...
	get #writes(): void {
		if (!this.#signer) throw new Error("The ledger connection is read-only");
		return;
	}
	...
	async deposit(assets: ChainAssets): Promise<UnsignedTxBatch> {
		...
		if (!assets.assets.size) throw new Error("attempting to deposit nothing");
		...
		const addStage = async (
			...
			if (chain == this.chain) {
				...
				tokenAddrAddr =
					(await this.#reader.getWrappedToken(assetID)) ??
					(() => {
						throw new Error(
							`Wrapped token for ${assetID} not yet deployed on ${getChainName(this.chain)}`,
						);
					})();
			}
			...
			if (amount instanceof Amount) {
				if (tokenAddrAddr.isZero() && chain === this.chain) {
					...
					calls.push(
						...(await this.#makeERC20DepositCalls(
							tokenAddrAddr,
							assetID,
							amount,
						)),
				...
				calls.push(
					...(await this.#makeERC721DepositCalls(
						tokenAddrAddr,
						assetID,
						amount,
					)),
	...
	async #makeERC20DepositCalls(
		...
		const holder =
			await this.#reader.tokenCache.getERC20Holder(contract_provider);
	...
	async #makeERC721DepositCalls(
		...
		const holder =
			await this.#reader.tokenCache.getERC721Holder(contract_provider);
