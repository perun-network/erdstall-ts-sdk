...
export class CodecWriter {
	#bytes: ArrayBuffer = new (ArrayBuffer as any)(1024, {
	...
	bool(v: boolean | Object | undefined) {
		this.u8(v ? 1 : 0);
	}
	u16(v: number) {
	...
	u256(v: bigint) {
		...
		this.#progress += 32;
		for (let i = 0; i < 32; i++) dst[i] = Number((v >> BigInt(i * 8)) & 0xffn);
	}
	...
	array_with<T>(array: T[], encode: (obj: T, w: CodecWriter) => void): void {
		...
		for (let it of array) encode(it, this);
	}

	map_with<K, V>(
		map: Map<K, V>,
		enc_k: (obj: K, w: CodecWriter) => void,
		enc_v: (obj: V, w: CodecWriter) => void,
	): void {
		this.array_with(Array.from(map.entries()), ([k, v]) => {
			enc_k(k, this);
			enc_v(v, this);
		});
	}
...
export class CodecReader {
	...
	u256(): bigint {
		...
		let ret = 0n;
		for (let i = 0; i < 32; i++) ret |= BigInt(src[i]) << BigInt(i * 8);
	...
	array<Dec>(decode: (r: CodecReader) => Dec): Dec[] {
		...
		for (let i = 0; i < ret.length; i++) ret[i] = decode(this);
		return ret;
	}
	...
	map<K, V>(
		...
		dec_v: (r: CodecReader) => V,
	): Map<K, V> {
		return new Map<K, V>(this.array<[K, V]>(() => [dec_k(this), dec_v(this)]));
	}

	rest(): Uint8Array {
		return this.#bytes.subarray(this.#progress);
	}
}
