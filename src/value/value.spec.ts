...
describe("values", () => {
	it("can be de- & encoded from & to JSON", () => {
		const vs = Values.fromJSON(testValues);
		expect(vs).eql(Values.fromJSON(JSON.stringify(vs)));
		// FIXME: One should not just compare string values like this, because
		// JSON is not order dependant.
		expect(JSON.stringify(vs)).eql(testValues);
	});
});
