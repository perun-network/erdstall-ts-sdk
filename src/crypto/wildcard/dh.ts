...
interface AESGCMKeyGen {
	name: "AES-GCM";
}
interface AESGCMKeyParams {
	name: "AES-GCM";
	length: 256;
}
interface DHDeriveParams {
	name: "X25519";
	public: DHPubKey;
}
...
declare namespace globalThis.crypto.subtle {
	...
	function exportKey(format: "raw", key: DHPubKey): Promise<ArrayBuffer>;

	function exportKey(format: "raw", key: AesGcmKey): Promise<ArrayBuffer>;
...
export class DHPK {
	...
	static decode(r: CodecReader): DHPK {
		return new DHPK(r.bytes(32));
	}
...
export class DHPair {
	...
	async deriveSharedKey(key: DHPK): Promise<AESGCMKey> {
		let secret = await deriveKey(
			{ name: "X25519", public: await key.nativeKey },
	...
	static get PK_LENGTH(): 32 {
		return 32;
	}

	encode_pk(w: CodecWriter): void {
		w.bytes<32>(this.#bytes!);
	}
}
...
export class AESGCMKey implements SigVerifier {
	...
	constructor(key: AesGcmKey) {
		this.#key = key;
	}

	static get TAG_BITLENGTH(): 128 {
		return 128;
	}
	static get KEY_LENGTH(): 32 {
		return 32;
	}
	...
	async encrypt(args: {
		...
		if (args.iv && args.iv.bytes.length !== InitVector.LENGTH)
			throw new Error(
				`Invalid IV size: ${args.iv.bytes.length} != ${InitVector.LENGTH}`,
			);
	...
	static async deserialise(key: Uint8Array): Promise<AESGCMKey> {
		...
		return new AESGCMKey(
			await importKey("raw", key, { name: "AES-GCM" }, true, [
				"encrypt",
				"decrypt",
			]),
		);
	...
	async verifySig(sig: SignedMessage): Promise<Uint8Array | undefined> {
		if (!(sig.signature instanceof InitVector)) return undefined;
...
export class InitVector extends Signature<"wildcard"> {
	constructor(public bytes: Uint8Array) {
		...
		if (bytes.length != InitVector.LENGTH)
			throw new Error(
				`Invalid IV size: ${bytes.length} != ${InitVector.LENGTH}`,
			);
	}

	static get LENGTH(): 12 {
		return 12;
	}

	static random(): InitVector {
		return new InitVector(getRandomValues(new Uint8Array(InitVector.LENGTH)));
	}
