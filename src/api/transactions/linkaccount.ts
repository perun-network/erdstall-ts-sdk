...
import {
	EthereumAddress, EthereumSigner, EthereumSignature
} from "#erdstall/crypto/ethereum";
import {
	SubstrateAddress, SubstrateSigner, SubstrateSignature
...
export class LinkAccount extends Transaction {
		...
		)
	{
		...
		if(accountID instanceof WildcardAddress)
	...
	async authorise_link(
		...
		subst: SubstrateSigner | undefined): Promise<void>
	{
		...
		if(this.eth_addr)
			this.eth_auth = await eth!.sign(auth);
		if(this.subst_addr)
			this.subst_auth = await subst!.sign(auth);
	}
...
export class LinkAccount_Output {
		constructor(
	public accountID: WildcardAddress
		) { }
