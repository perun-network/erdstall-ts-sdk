...
export abstract class NonceCheck {
	static decode(r: CodecReader): NonceCheck {
		...
		default: throw new Error(`Unknown nonce check type ${t}`);
		}
	}
...
export class NoNonceCheck extends NonceCheck {
	...
	override nonceCheckType(): NonceCheckType { return NonceCheckType.None; }
	static decode_impl(r: CodecReader): NoNonceCheck
		{ return new NoNonceCheck(); }
};
export class StrictNonceCheck extends NonceCheck {
	constructor(public nonce: bigint) { super(); }
	override encode_impl(w: CodecWriter): void { w.u64(this.nonce); }
...
export class SignedTransaction<Tx extends Transaction = Transaction> extends ErdstallObject {
		constructor(
	...
	override objectType() { return SignedTransaction; }
	override objectTypeName(): string { return transactionTypeName; }
...
export class TransactionOutput {
	...
	encode(w: CodecWriter): void { TransactionOutput.encode(w, this); }
	static encode(w: CodecWriter, self: TransactionOutput)
		{ w.u16(self.payload.length); w.bytes(self.payload); }
	static decode(r: CodecReader): TransactionOutput
		{ return new TransactionOutput(r.bytes(r.u16())); }

	reader(): CodecReader { return new CodecReader(this.payload); }
}