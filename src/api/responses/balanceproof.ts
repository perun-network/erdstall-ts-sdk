...
export class SignedBalanceProof extends ErdstallObject {
		constructor(
	readonly sig: SignedMessage<BalanceProof>
		) { super(); }
	...
	override objectType() { return SignedBalanceProof; }
	override objectTypeName(): string { return balanceProofsTypeName; }
	...
	encode(w: CodecWriter): void { SignedBalanceProof.encode(w, this); }
...
export class BalanceProof {
	...
	encode(w: CodecWriter): void { BalanceProof.encode(w, this); }
	static encode(w: CodecWriter, self: BalanceProof): void
	{
		w.map_with(self.proofs,
			(addr) => Address.encode(w, Address.fromKey(addr)),
			(proofs) => w.map_with(proofs,
				(chain) => w.u16(chain),
...
export class ChainProofChunk {
	...
	encode(w: CodecWriter): void { ChainProofChunk.encode(w, this); }
...
export class ChainProof {
	...
	encode(w: CodecWriter): void { ChainProof.encode(w, this); }
