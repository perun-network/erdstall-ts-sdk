...
describe("Erdstall-TS-SDK", () => {
	...
	before(async () => {
		fs.writeFileSync("operator.cfg", JSON.stringify({
			Mnemonic: mnemonic,
		...
		return new Promise((accept, reject) => {
			const process = exec(["$ERDSTALL_EXE",
				"-config operator.cfg",
				"-epochs 10",
				`-ledger-port ${nodePort}`
			].join(" "), e => { if(e) reject(e); });
	...
	it("deposits", async() => {
		const depositBal = new Assets({
			token: assets.ETHZERO, asset: new assets.Amount(100000000n)
		});
	...
	it("leave", async() => {
		const stages = await Promise.all(clients.slice(0,2).map(c => c.leave()));
		return Promise.all(stages.map(stage => stage.wait()));
	});
