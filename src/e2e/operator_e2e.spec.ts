...
import { mkDefaultEthereumSessionConstructor } from "#erdstall/ledger/backend/ethereum";
...
function e2e_operator(operator_url?: URL) {
	const { ethSession, substSession } = describeV(
		...
		() => {
			...
			const ethSession = itAsync(
				"Setting up eth session",
				async () =>
			...
			const substSession = itAsync(
				"Setting up substrate session",
				async () =>
			...
			itAsync("Subscribing to receipts", async () =>
				Promise.all([
	...
	for (let i = 0; i < 2; i++)
		describeV(
			`Minting a token on Ethereum account${i ? " again (should fail)" : ""}`,
			() => {
				let handle = itAsync(
					"Sending transaction",
					async () => await (await ethSession).mint(tokenName, 1n),
				);
				itAsync(
					"Awaiting confirmation",
					async () => await (await handle).accepted,
				);
				itAsync(
					`Expecting ${i ? "failed" : "successful"} receipt`,
					async () => {
	...
	describeV(
		"Transferring minted token to Substrate account",
		() => {
			const acc = itAsync(
				"Fetching balance",
				async () => await (await ethSession).getOwnAccount(),
			);
			const handle = itAsync(
				"Sending transaction",
				async () =>
					await (
						await ethSession
					).transferTo((await acc).values, (await substSession).address),
			);
	...
	for (let i = 0; i < 2; i++)
		describeV(
			`Transferring minted token back to Ethereum account${i ? " again (should fail)" : ""}`,
			() => {
				const handle = itAsync("Sending transaction", async () => {
					...
					return await (
						await substSession
					).transferTo(values, (await ethSession).address);
				});
				itAsync("Awaiting confirmation", async () => (await handle).accepted);
